...
from aicssegmentation.core.pre_processing_utils import intensity_normalization, edge_preserving_smoothing_3d,image_smoothing_gaussian_slice_by_slice, image_smoothing_gaussian_3d
...
class cell_segment:
    ...
    def img_norm_smooth(self):
        ...
        for t in pb:
            ...
            self.structure_img_smooth[t] = image_smoothing_gaussian_3d(img, sigma=self.smooth_sigma)
    ...
    def threshold_Time_KDEL(self):
        ...
        for t in pb:
            ...
            bw = binary_opening(bw, footprint=np.ones((1, 1, 1)))

            bw = hole_filling(bw, 1, 5, fill_2d=True)
            bw = remove_small_objects(bw > 0, min_size=50, connectivity=1)
            ...
            if t == 0:
             ...
             #print(mid_z)
                ...
                if abs(mid_z_temp - mid_z) < 8:
                    mid_z = mid_z_temp
            
            self.mid_z_list.append(mid_z)
            bw_mid_z = bw[mid_z, :, :]
            bw_mid_z = remove_small_objects(bw_mid_z > 0, min_size=30, connectivity=1)

            self.bw_midplane[t, mid_z, :, :] = label(bw_mid_z)

            # Get seed map, do a 3D watershed for illustration purpose. 3D info is not used in segmentation,instead 2D mid plane is used for segmentation
            ...
            seg = remove_small_objects(seg > 0, min_size=200, connectivity=1)
    ...
    def threshold_Time_Sec61B(self):
        ...
        for t in pb:
            ...
            if t !=8:
              ...
              bw = remove_small_objects(bw > 0, min_size=300, connectivity=1)
              ...
              bw_mid_z = bw[mid_z, :, :]
              bw_mid_z = remove_small_objects(bw_mid_z > 0, min_size=50, connectivity=1)
    ...
    def Normalize(self):
        ...
        area_norm = np.median(self.positions_table.loc[self.positions_table.frame == 0, "area"])
        perimeter_norm = np.median(self.positions_table.loc[self.positions_table.frame == 0, "perimeter"])
        circularity_norm = np.median(self.positions_table.loc[self.positions_table.frame == 0, "circularity"])
    ...
    def plotting(self, napari_integration=True):
        ...
        if napari_integration == True:
            ...
            for t in range(self.image.shape[0]):
                ...
                area = np.median(df_subset['normalized area'])
                perimeter = np.median(df_subset['normalized perimeter'])
                circularity = np.median(df_subset['normalized circularity'])
            ...
            axes[2].plot(self.frame_list, self.norm_circularity_list, color=color[3], marker='o')
